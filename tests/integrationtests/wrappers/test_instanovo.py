...
def test_instanovo_command_construction_yaml_no_model(tmp_path: Path) -> None:
    ...
    # find_instanovo_configs() locates instanovo's own venv, which isn't
    # importable from this test process (instanovo lives in a separate
    # instanovo_only_venv). Mock the configs path directly rather than
    # constructing a real venv on disk -- os.path.relpath is purely
    # string-based, so the path doesn't need to exist.
